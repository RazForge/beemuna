...
export default function AIPage() {
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-4rem)] max-w-6xl flex-col gap-4">
      <header className="glass rounded-[28px] p-6 flex flex-wrap items-center justify-between gap-4 shadow-xl border-white/20 dark:border-white/5">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <select
            ...
            className="h-10 rounded-full border border-black/10 bg-white/50 px-4 text-sm font-medium shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-black/20 focus:outline-none"
          >
            {modes.map((m) => (
              <option key={m.value} value={m.value} className="bg-card text-foreground">{t(m.labelKey)}</option>
            ))}
          </select>
          <select
            value={spaceId}
            onChange={(e) => setSpaceId(e.target.value)}
            className="h-10 max-w-48 rounded-full border border-black/10 bg-white/50 px-4 text-sm font-medium shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-black/20 focus:outline-none"
          >
      ...
      <div className="flex flex-1 gap-4 overflow-hidden">
        <div className="w-60 shrink-0 space-y-2 overflow-y-auto rounded-2xl border border-border bg-card p-3">
          {conversationsQuery.data?.map((conv) => (
            <div
              key={conv.id}
              className={cn(
                "group flex cursor-pointer items-center gap-2 rounded-xl px-3 py-2 text-sm transition-colors",
                activeConv === conv.id ? "bg-primary/12 text-primary" : "hover:bg-accent",
              )}
            ...
            >
              ...
              <button
                ...
                className="hidden h-6 w-6 items-center justify-center rounded text-destructive hover:bg-destructive/10 group-hover:flex"
              >
          ...
          <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto p-6">
                ...
                >
                  ...
                  {m.role === "assistant" && m.sources.length > 0 && (
                    <div className="mt-3 space-y-1 border-t border-border/60 pt-2">
                      ...
                      {m.sources.map((s, i) => (
                        <div key={i} className="rounded-lg bg-card/70 p-2">
            ...
            {streaming && (
              <div className="flex justify-start">
                <div className="max-w-[80%] rounded-2xl rounded-bl-sm bg-muted px-4 py-3 text-sm leading-relaxed">
                  {streamText ? (
          ...
          <div className="border-t border-border p-4">
            <div className="flex items-end gap-2">
              <textarea
                ...
                disabled={!activeConv}
                rows={1}
                className="min-h-11 flex-1 resize-none rounded-full border border-input bg-background px-4 py-3 text-sm focus-ring disabled:opacity-50"
              />
              {streaming ? (
                <Button size="icon" className="h-11 w-11 shrink-0" variant="outline" onClick={handleStop} title={t("stop")}>
